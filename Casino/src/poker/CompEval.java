...
public class CompEval implements CompInterface{
	...
	public Comparison comp(Hand hand){
		Card[] myHand = new Card[hand.size()];
		...
		if(isStraightFlush(myHand) && myHand[0].getType() == Card.Rank.ACE &&
				myHand[1].getType() == Card.Rank.KING){
	...
	private boolean isTwoPair(Card[] myCards){	
		if(myCards[0].compareCards(myCards[1]) != 0)
		{
