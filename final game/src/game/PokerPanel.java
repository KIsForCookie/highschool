...
/*
 * poker does not work at the moment
 * check PokerTest class for proof of concept
 */
...
public class PokerPanel extends JPanel implements ActionListener, MouseListener{
	...
	JButton twentyFive, fifty, hundred, twoFifty, fiveHundred, thousand, 
		twentyFiveHundred, fiveGrand, tenGrand;
	ImageIcon twentyFiveimg, fiftyimg, hundredimg, twoFiftyimg, fiveHundredimg, 
		thousandimg, twentyFiveHundredimg, fiveGrandimg, tenGrandimg;
	JLabel background;
	JButton call, check, raise, allIn, fold, go, leave, confirm, back, discard, cont;
	...
	double cardwidth = new PokerCard(Rank.ACE, Suit.SPADES).getBack().getWidth();
	double cardheight = new PokerCard(Rank.ACE, Suit.SPADES).getBack().getHeight();
	...
	ArrayList<PokerCard> discards = new ArrayList<PokerCard>();
	...
	private class CTimer implements ActionListener{
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if(Table.pot.turnsNoRaise >= Table.table.size()){
			...
			if(Table.pot.betsMade == true){
				check.setVisible(false);
				call.setVisible(true);
			}
			
			if(Table.pot.betCount > 0){
				Table.pot.betsMade = true;
			}else{
				Table.pot.betsMade = false;
			}
			...
			if(played){
				...
				for(int i = 0; i < table.size(); i++){
					...
					if(table.get(i).Act()[1] < Table.pot.minimumBet)
						table.get(i).Act()[1] = Table.pot.minimumBet;
			...
			if(Table.pot.round == 2){
				discard.setVisible(true);
				discard.setBounds(900, 600, 75, 25);
			}
	...
	public void paint(Graphics g) {
		...
		//g2d.drawString("# bets: " + Table.pot.betCount, 25, 25);
		scale.scale(1,1);		
		...
		//all chips
		if((table.size() > 0 && table.pot.bets.size() > 0) || table == null){	
			for(int i = 0; i < table.size(); i++){
				table.getPot().bet(table.get(i).getChips(), table.get(i).playerNum, table.get(i).isAllIn());
				table.get(i).getChips().clear();
			}
			for(int i = 0; i < table.pot.bets.size(); i++){
				for(int j = 0; j < table.pot.bets.get(i).size(); j++){
					x.add(new Integer((int)(Math.random() * 256 + 352)));
					y.add(new Integer((int)(Math.random() * 128 + 256)));
					scale.drawImage(table.pot.bets.get(i).get(j).getBImg(), x.get(x.size() - 1), y.get(y.size() - 1), null);
				}
			}
		}
		if(showCPUCards == true){
			if(cardsFaceUp == true){
				...
				if(table.get(1) != null && PlayerPos != 1){
					...
					for(int j = 0; j < table.get(1).getHand().size(); j++){
						temp.add(rotate(table.get(1).getHand().getCard(j).getImage(), 2));
						//temp.add(rotate(rotate(table.get(1).getHand().getCard(j).getImage(), 1), 1));
					}
				...
				if(table.get(1) != null && PlayerPos != 1){
					...
					for(int j = 0; j < table.get(1).getHand().size(); j++){
						temp.add(rotate(table.get(1).getHand().getCard(j).getBack(), 2));
						//temp.add(rotate(rotate(table.get(1).getHand().getCard(j).getImage(), 1), 1));
					}
					for(int i = 0; i < temp.size(); i++){
						scale.drawImage(temp.get(i), 350 + i *75, 50, null);
			...
			if(Table.pot.getPotValue() > 0){
				g2d.setColor(Color.YELLOW);
				g2d.setFont(new Font(null, Font.PLAIN, 24));
				g2d.drawString("Pot total: $" + table.getPot().getPotValue(), 25,25);
			}
		}
	...
	/* How the game goes
	 * 
	 * players ante, get dealt cards				[x]
	 * raise, check, call, etc						[x]
	 * continues until one round (everyone goes)	
	 * completes with no one raising				[ ]
	 * 
	 * discard up to 3 cards						[x]
	 * 
	 * raise, check, call, etc						[x]
	 * continues until one round (everyone goes)	
	 * completes with no one raising				[ ]
	 * 
	 * hands are shown for those still in			[x]
	 * hands compared, winner takes the pot			[ ]
	 * 
	 * repeat ad infinitum
	 * 
	 */
	...
	public PokerPanel(){
		...
		//for(int i = 1; i < k + 1; i++){
			...
			table.get(1).generateName();
			table.get(1).setCompPlayer();
	...
	public void actionPerformed(ActionEvent e){
		...
		else if(e.getSource() == allIn){	//commented out due to freezing issues
			/*
			check.setVisible(false);
		...
		if(e.getSource() == confirm){
			...
			int bValue = 0;
			for(int i = 0; i < PlayerChips.size(); i++){
				bValue += PlayerChips.get(i).getValue();
			}
			if(bValue > Table.pot.minimumBet){
				Table.pot.minimumBet = bValue;
			}
			PlayerChips.clear();
			AISGo();
			played = true;
		...
		if(e.getSource() == discard){
			table.discardTime(table.get(PlayerPos).discard());
		}
	}
